...
import { getSongList, getTopList,getSongUrlByMid } from "api/api.js";
...
export default {
  ...
  async created() {
    ...
    //赋值  歌曲  榜名  头像
    this.Alllist = songlist;
    this.name = topinfo.ListName;
    this.avator = `https://y.gtimg.cn/music/photo_new/${topinfo.pic_album.substring(39)}`;
    // 处理songlist数据
    let {result,mids} = this.handleList(songlist);
